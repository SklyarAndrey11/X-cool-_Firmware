...
DComplex* solve_quartic(double a, double b, double c, double d)
{
    ...
    static DComplex retval[4];

	// solving quadratic eq. - x^2 + p1*x + q1 = 0
	D = p1*p1 - 4*q1;
	if(D < 0.0)
	{
		retval[0] = creal( -p1 * 0.5 );
		retval[0] = cimag( sqrt(-D) * 0.5 );
		retval[1] = conj(retval[0]);
	}
	else
	{
		sqD = sqrt(D);
		retval[0] = creal( (-p1 + sqD) * 0.5 );
		retval[1] = creal( (-p1 - sqD) * 0.5 );
	}
	...
	if(D < 0.0)
	{
		retval[2] =  creal( -p2 * 0.5 );
		retval[2] = cimag( sqrt(-D) * 0.5 );
		retval[3] = conj(retval[2]);
	}
	else
	{
		sqD = sqrt(D);
		retval[2] = creal( (-p2 + sqD) * 0.5 );
		retval[3] = creal( (-p2 - sqD) * 0.5 );
	}
